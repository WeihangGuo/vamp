...
namespace vamp::collision
{
    ...
    template <typename DataT>
    inline constexpr auto sphere_capsule_l2(
        const Capsule<DataT> &c,
        const DataT &x,
        const DataT &y,
        const DataT &z,
        const DataT &r) noexcept -> DataT
    {
        ...
        auto sum = sql2_3(x, y, z, c.x1 + c.xv * cdf, c.y1 + c.yv * cdf, c.z1 + c.zv * cdf).sqrt();
        auto rs = r + c.r;
        return sum - rs;
    }
    ...
    {
        return sphere_capsule_l2(c, s.x, s.y, s.z, s.r);
    }

    template <typename DataT>
    inline constexpr auto sphere_z_aligned_capsule_l2(
        const Capsule<DataT> &c,
        const DataT &x,
        const DataT &y,
        const DataT &z,
        const DataT &r) noexcept -> DataT
    {
        ...
        auto sum = sql2_3(x, y, z, c.x1, c.y1, c.z1 + c.zv * cdf).sqrt();
        auto rs = r + c.r;
        return sum - rs;
    }
    ...
    inline constexpr auto sphere_z_aligned_capsule_l2(const Capsule<DataT> &c, const Sphere<DataT> &s) noexcept
        -> DataT
    {
        return sphere_z_aligned_capsule_l2(c, s.x, s.y, s.z, s.r);
    }
}  // namespace vamp::collision
